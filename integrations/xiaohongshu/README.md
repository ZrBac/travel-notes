...
- 检查只打开一次首页，最多点击一次登录入口。判断页面可见控件，不以所有资源加载完成作为唯一条件。
- 页面出现 300012 / 安全验证 / 安全限制，或文档与接口返回 403，以及任何 429，立即停止。
- 最长执行 60 秒；systemd 限制到一个 CPU 核、768 MiB 内存、192 个任务，退出时清理整个进程组。

`control.json` 为冷却记录；`last-run.json` 为最近实际浏览器检查结果；`last-network.json` 仅记录请求类型、主机名、响应状态与相对时间，不记录 URL 参数、Cookie、请求/响应正文或认证头。每次覆盖上次统计，不累计膨胀。正常浏览器页面缓存保留，没有使用会禁用 HTTP 缓存的全局请求拦截。
