...
(async()=>{const b=await chromium.launch({args:['--no-sandbox']});try{
 for(const width of [1440,390,320]){
  ...
  assert.ok(await p.locator('.media-card img').evaluateAll(nodes=>nodes.every(n=>n.getAttribute('data-admin-src').endsWith('?w=640')&&n.loading==='lazy')));
  ...
  assert.ok(await p.locator('.picker-card img').evaluateAll(nodes=>nodes.every(n=>n.getAttribute('data-admin-src').endsWith('?w=640')&&n.loading==='lazy')));
  ...
  await p.locator('.media-toolbar [data-action=media-refresh]').click();await p.waitForFunction(()=>!document.querySelector('[data-action=media-refresh]').disabled);assert.equal(mediaCalls,coldCalls+2,'manual refresh bypasses cached data');
