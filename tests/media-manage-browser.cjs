...
const root=process.env.TRAVEL_BROWSER_SOURCE||path.resolve(__dirname,'..'),pixel=Buffer.from('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+/lL8AAAAASUVORK5CYII=','base64');
(async()=>{const b=await chromium.launch({args:['--no-sandbox']});try{for(const width of [1440,768,390,320]){
 const context=await b.newContext({viewport:{width,height:1000}}),p=await context.newPage(),errors=[],operations=[];let accept=true,mediaCalls=0,failList=false,failWrite=false;
 p.setDefaultTimeout(8000);
 await p.addInitScript(()=>{
  const timer=window.setTimeout;window.setTimeout=(fn,ms,...args)=>timer(fn,ms===15000?900:ms,...args);
  const fetch=window.fetch;window.fetch=(url,options={})=>{
   if(window.slowMedia&&url==='/api/admin/media')return new Promise((resolve,reject)=>options.signal.addEventListener('abort',()=>reject(new DOMException('Aborted','AbortError')),{once:true}));
 ...
 if(u.pathname.startsWith('/api/')){
  ...
  if(u.pathname==='/api/admin/media'){mediaCalls++;return failList?r.fulfill({status:503,json:{error:'模拟列表刷新失败'}}):r.fulfill({json:{media,defaults:[{url:'/static/assets/lake.jpg',name:'湖畔'}]}});}
  ...
  if(u.pathname.startsWith('/api/admin/media/')){
   ...
   if(failWrite)return r.fulfill({status:409,json:{error:'图片仍在使用，请刷新列表查看'}});
 ...
 failList=true;await p.evaluate(()=>{window.unchangedImage=document.querySelector('.media-card img');});
 ...
 assert.equal(mediaCalls,1,'successful deletion must not wait for another list request');assert.ok(await p.evaluate(()=>unchangedImage===document.querySelector('.media-card img')&&unchangedImage.isConnected),'unchanged images stay attached');
 ...
 assert.equal(mediaCalls,1,'delete/restore/purge and local tabs need no extra metadata requests');
 await p.locator('[data-action=media-tab][data-value=active]').click();await p.waitForSelector('.media-card');
 const beforeFailure=await p.locator('.media-card').count();failWrite=true;
 await p.locator('[data-action=media-update][data-value=trash]:not(:disabled)').first().click();await p.waitForFunction(()=>!state.mediaBusy&&document.querySelector('#toast').textContent.includes('图片仍在使用'));
 assert.equal(await p.locator('.media-card').count(),beforeFailure,'failed write preserves cards');failWrite=false;
 await p.evaluate(()=>{window.slowMedia=true;});await p.locator('[data-action=media-refresh]').click();await p.waitForSelector('#media-refresh-status:not([hidden])');
 assert.equal(await p.locator('.media-card').count(),beforeFailure,'refresh timeout preserves existing content');assert.equal(await p.locator('#main .initial-loading').count(),0);assert.equal(await p.locator('[data-action=retry-page]').count(),0);
 await p.evaluate(()=>{window.slowMedia=false;});failList=false;
 await p.locator('[data-action=media-refresh]').click();await p.waitForFunction(()=>!document.querySelector('[data-action=media-refresh]').disabled&&document.querySelector('#media-refresh-status').hidden);
 assert.equal(await p.locator('.media-card').count(),beforeFailure);
 // A refresh already in flight must never resurrect an image after deletion.
 await p.evaluate(()=>{window.slowMedia=true;});await p.locator('[data-action=media-refresh]').click();
 const deleted=await p.locator('.media-check:not(:disabled)').first().inputValue();await p.locator('[data-action=media-update][data-value=trash]:not(:disabled)').first().click();await p.waitForFunction(()=>!state.mediaBusy&&document.querySelector('#toast').textContent==='已将 1 张图片移入回收站');
 assert.equal(await p.locator('.media-check[value="'+deleted+'"]').count(),0);await p.evaluate(()=>{window.slowMedia=false;});
...
}console.log('PASS: clear labels, no header refresh, usage links, protected used images, selection/cancel/delete/restore/purge/page reset, unified trash tabs, no reload after mutations, retained images, failed refresh preservation/retry/race, desktop and mobile');}finally{await b.close();}})().catch(e=>{console.error(e);process.exit(1)});
