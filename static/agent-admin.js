...
const TravelAgent = (()=>{
  let revision=0, timer=null, selected=null, generation=0, busy=false, service={}, working=false, tasks=[], nextBefore=null, expanded=false;const checked=new Set();
  ...
  function stop(){TaskManager.cancel();checked.clear();clearTimeout(timer);timer=null;generation++;document.querySelector('#main')?.removeEventListener('click',handle);}
  async function page(gen){
    stop();const token=generation;selected=null;tasks=[];expanded=false;
    ...
    renderOverview();renderTasks();bind();schedule(token);
  }
  ...
  function renderTasks(){
    ...
    TaskManager.update('agent',tasks,checked);$('#agent-older').hidden=!nextBefore;
  }
  ...
  async function refresh(token=generation){
    ...
    service=data.service;working=data.tasks.some(t=>active(t.status));tasks=expanded?[...new Map([...tasks,...data.tasks].map(t=>[t.id,t])).values()].sort((a,b)=>b.id-a.id):data.tasks;if(!expanded)nextBefore=data.next_before;renderOverview();renderTasks();
    ...
    if(selected&&(!current||active(current.status)||$('#agent-detail').dataset.updated!==current.updated_at)){const id=selected;let detail;
      try{detail=await api('/admin/agent/tasks/'+id);}catch(error){
        ...
        forgetTask(id);tasks=tasks.filter(t=>t.id!==id);renderTasks();return;
      }
  ...
  async function handle(e){
    ...
    try{
      if(TaskManager.select('agent',action,node,tasks,checked))return;
      ...
      if(action==='delete'||action==='delete-selected'){
        ...
        const result=await TaskManager.remove('/admin/agent',ids);if(token!==generation||!result)return;
        const removed=new Set(result.ids);tasks=tasks.filter(t=>!removed.has(t.id));for(const id of removed)checked.delete(id);
        if(removed.has(selected))forgetTask(selected);
        const form=$('#agent-task-form');if(removed.has(Number(form.dataset.parent))){delete form.dataset.parent;$('#agent-followup-note').textContent='关联记录已删除，当前填写内容将作为新任务提交。';}
