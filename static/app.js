...
function guidesHome() {
  updateShell('guides'); const featured=state.guides.find(g=>!g.sample)||state.guides[0];
  const countries=[...new Set(state.guides.map(g=>g.country).filter(Boolean))];
  const destinations=new Set(state.guides.map(g=>g.destination)).size;
  $('#app').innerHTML=heading('下一站，心之所向。',state.site?.tagline||'收藏旅途的灵感，把每一次出发都安排得刚刚好。')+autumnBanner()+
    (featured?`<section class="hero-grid"><a href="#guide/${featured.id}" class="hero"><img class="hero-image" src="${esc(travelImageUrl(featured.cover,1280))}" alt="${esc(featured.destination)}旅行封面" fetchpriority="high"><span class="hero-badge">${icon('spark')}精选旅行灵感</span><div><div class="hero-kicker">A LITTLE ESCAPE, A LOT OF MEMORIES</div><h2>${esc(featured.title)}</h2><div class="hero-meta"><span>${icon('pin')} ${esc(featured.destination)}</span><span>·</span><span>${featured.days} 天慢时光</span><span>·</span><span>打开这篇攻略</span></div></div><span class="hero-arrow">${icon('up')}</span></a><aside class="collection-note"><div><div class="note-top">${icon('book')}我的旅行收藏夹</div><h2>一篇攻略，<br>一段出发的期待。</h2><p class="note-copy">让零散的灵感有处安放，<br>让想去的远方，慢慢有了模样。</p></div><div class="stats"><div class="stat"><strong>${String(state.guides.length).padStart(2,'0')}</strong><span>篇旅行攻略</span></div><div class="stat"><strong>${String(destinations).padStart(2,'0')}</strong><span>个心动目的地</span></div></div></aside></section>`:'')+
    `<section aria-label="攻略收藏"><div class="collection-toolbar"><div class="section-title"><h2>${esc(state.destination||state.tag||'我的攻略')}</h2><span class="count-pill" id="result-count">${state.guides.length}</span></div><div class="search-sort"><div class="search-box">${icon('search')}<input id="search" type="search" aria-label="搜索攻略" placeholder="搜索目的地、攻略或标签…" value="${esc(state.query)}"></div><select id="sort" class="sort-select" aria-label="排序方式"><option value="updated">最近更新</option><option value="title">标题排序</option><option value="days">旅行天数</option></select></div></div><div class="filter-row"><div class="chips"><button class="chip ${!state.country?'active':''}" data-action="country" data-value="">全部目的地</button>${countries.map(c=>`<button class="chip ${state.country===c?'active':''}" data-action="country" data-value="${esc(c)}">${esc(c)}</button>`).join('')}${state.tag||state.destination?'<button class="chip" data-action="clear">清除筛选 ×</button>':''}</div><span class="filter-hint">旅行的意义，藏在每一个细节里</span></div>${state.admin?`<div class="state-tabs">${[['','全部状态'],['public','公开'],['private','私密'],['draft','草稿']].map(([v,l])=>`<button class="chip ${state.status===v?'active':''}" data-action="status" data-value="${v}">${l}</button>`).join('')}</div>`:''}<div class="guides-grid" id="guide-grid"></div><div class="end-note">世界很大，慢慢出发</div></section>`;
...
function destinations(){
  ...
  $('#app').innerHTML=heading('把远方，一一收藏。','按目的地整理攻略，让每一个想去的地方都有自己的位置。','PLACES THAT STAY WITH YOU')+`<div class="destination-grid">${[...groups].map(([name,list])=>`<button class="destination-card" data-action="destination" data-value="${esc(name)}"><img src="${esc(travelImageUrl(list[0].cover,640))}" alt="${esc(name)}旅行封面" loading="lazy"><p>${esc(list[0].country||'下一站')}</p><h2>${esc(name)}</h2><p>${list.length} 篇攻略 · 探索这个目的地 ↗</p></button>`).join('')||empty('目的地清单还是空的','新建攻略时填写目的地，这里就会自动整理。')}</div>`;
}
function tags(){updateShell('tags');const groups=new Map();state.guides.forEach(g=>g.tags.forEach(t=>groups.set(t,(groups.get(t)||0)+1)));$('#app').innerHTML=heading('跟着心情，寻找灵感。','山野、海边、城市小巷，每一种向往都有自己的标签。','LITTLE IDEAS, BIG ADVENTURES')+`<div class="tag-grid">${[...groups].sort((a,b)=>b[1]-a[1]).map(([t,n])=>`<button class="tag-tile" data-action="tag" data-value="${esc(t)}">${icon('tag')}<span><strong>${esc(t)}</strong><small>${n} 篇相关攻略</small></span></button>`).join('')||empty('等待第一份灵感','编辑攻略时添加标签，方便下次找到它。')}</div>`;}
