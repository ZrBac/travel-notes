...
攻略数据与分类并行读取；列表、统计和素材元数据在当前标签页内最多保留 10 秒、12 组，菜单悬停／聚焦时按需预取。素材搜索栏旁“刷新列表”可立即重新读取。没有使用 localStorage、sessionStorage 或 Service Worker 保存后台数据；隐藏标签页、退出或会话失效时清理，写入前后清理并通知其他后台标签页。编辑正文、设置、会话和助手实时状态始终重新读取。HTTP 接口仍为 no-store，私密权限仍由服务器验证。
