...
访客网站提供 PWA：手机底部和电脑主导航均为首页、攻略、足迹；右上角「更多」收纳已保存攻略、安装及目的地/标签浏览。通过「更多 → 添加到主屏幕」查看安装指引；iPhone 在 Safari 分享菜单中添加，在独立窗口内保存攻略。公开攻略详情可手动保存或更新正文及 640px 配图，在「更多 → 已保存攻略」中打开、更新、移除。每台设备最多保存 20 篇、100 MB；单篇图片预算 25 MB、最多 80 张、并发下载 2 张。缺失图片明确标注，取消或整篇保存失败不会覆盖旧副本。仅有链接、外部资料和关联足迹不包含在离线包内。
