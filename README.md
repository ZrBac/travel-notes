...
后台只在分类相关页签读取分类信息；切页取消旧的读取请求，读取接口及响应正文超过 15 秒显示超时与重新加载入口。上传和保存不受读取超时或切页取消影响。接口仍禁止缓存，私密内容继续经过权限校验。

工作台、攻略列表、素材库及图片选择器使用 640 像素缩略图，素材预览使用 1280 像素图片，选中素材仍保留原始地址。素材库每页 24 张，搜索和翻页复用元数据，减少图片并发占用带宽。

离线回归：`node tests/admin-loading-browser.cjs`，覆盖分页与搜索、缩略图、切页取消、响应头／正文超时重试、写入不被取消、登录失效及手机布局。
